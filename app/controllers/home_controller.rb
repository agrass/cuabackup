class HomeController < ApplicationController
  ...
  def ingredientsPanel
    if params[:date]
      ...
      @ingredients = Ingredient.joins(plates: [:areas, orders: :order_list]).where(order_lists: {fecha: date[2]+'-'+date[1]+'-'+date[0]}, regime_plates: { :horario => horario.to_i, :dia =>  DayRegime.getDayByDate(Date.parse(fecha)) }).select('ingredients.nombre, sum(plate_ingredients.cantidad) as cantidad_total, ingredients.unidad, orders.horario as horario, areas.id as area_id').group('ingredients.nombre, areas.id, ingredients.unidad, orders.horario').order('areas.id, orders.horario')
